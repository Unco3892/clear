...
import pandas as pd
# from sklearn_quantile import SampleRandomForestQuantileRegressor, KNeighborsQuantileRegressor
...
class uacqr():
    def __init__(self, model_params=dict(),
                 B=100, q_lower=5, q_upper=95, alpha=0.05, transform=None, inv_transform=None,
                 ...
                 uacqrs_bagging=False, double_bootstrapping=False, n_jobs=-1):
        '''
        inputs:
        ...
        n_jobs (int): number of parallel jobs for bootstrap model fitting (default: -1 = all cores)
        '''
    ...
    def _fit_single_bootstrap(self, x_train, y_train, b_idx, subsample=1.0):
        """
        ...
        Args:
            ...
            b_idx: Bootstrap index
            subsample: Subsample ratio
            
        Returns:
            A fitted model for this bootstrap iteration
        """
        # Create bootstrap sample
        b_idxs = np.random.choice(y_train.shape[0], size=int(y_train.shape[0]*subsample), replace=True)
        ...
        # Create and fit model based on model_type
        if self.model_type=='rfqr':
            # if self.sample_rfqr:
            #     model_b = SampleRandomForestQuantileRegressor(default_quantiles=[self.q_lower/100, self.q_upper/100], 
            #                                      random_state=self.random_state+b_idx, 
            #                                      n_jobs=1, **self.uacqrp_model_params)
            # else:
            model_b = RandomForestQuantileRegressor(default_quantiles=[self.q_lower/100, self.q_upper/100], 
                                                random_state=self.random_state+b_idx, 
                                                n_jobs=1, **self.uacqrp_model_params)
            ...
            model_b_lower = LGBMRegressor(objective='quantile', metric='quantile', boosting_type='gbdt',
                                        alpha=self.q_lower/100, n_jobs=1, random_state=self.random_state+b_idx, **self.uacqrp_model_params)
            model_b_upper = LGBMRegressor(objective='quantile', metric='quantile', boosting_type='gbdt',
                                        alpha=self.q_upper/100, n_jobs=1, random_state=self.random_state+b_idx, **self.uacqrp_model_params)
            model_b_lower.fit(x_train_b, y_train_b)
            model_b_upper.fit(x_train_b, y_train_b)
            model_b = [model_b_lower, model_b_upper]
        ...
        #     model_b = KNeighborsQuantileRegressor(default_quantiles=[self.q_lower/100, self.q_upper/100], **self.uacqrp_model_params)
        #     model_b.fit(x_train_b, y_train_b)
            ...
            model_b = QuantileRegressionNN(quantiles=self.modeled_quantiles, 
                                            random_state=self.random_state+b_idx, **self.uacqrp_model_params)
            model_b.fit(x_train_b, y_train_b)
        elif self.model_type =='ls':
            model_b = LinearRegression(n_jobs=1)
            model_b.fit(x_train_b, y_train_b)
            # Note: sigmahat will be calculated separately after all models are fit
        else:
            # For unsupported model types in parallel mode
            model_b = None
    ...
    def fit(self, x_train, y_train):
        ...
        # Special case handling for neural_net without bootstrapping and catboost
        if (self.model_type == 'neural_net') and not(self.bootstrapping_for_uacqrp):
            ...
            self.models_B = []  # Initialize empty models list
            ...
            # Parallelize bootstrap model fitting for other model types
            print(f"Fitting {self.B} bootstrap models in parallel...")
            # Determine number of jobs based on available cores and model type
            # Use fewer jobs for more complex models to avoid memory issues
            # Use self.n_jobs if provided, otherwise auto-determine based on model type
            if self.n_jobs > 0:
                n_jobs = self.n_jobs
            else:
                # Auto-determine based on model complexity
                if self.model_type in ['neural_net', 'lightgbm']:
                    n_jobs = min(joblib.cpu_count() // 2, 4)
                else:
                    n_jobs = min(joblib.cpu_count(), 8)
                
            # Set individual model n_jobs=1 to avoid nested parallelism
            print(f"Using {n_jobs} parallel jobs for bootstrap model fitting")
            
            try:
                # Fit bootstrap models in parallel
                self.models_B = joblib.Parallel(n_jobs=n_jobs, verbose=10)(
                    joblib.delayed(self._fit_single_bootstrap)(
                        x_train, y_train, b, self.subsample
                    ) for b in range(self.B)
                )
                
                # Filter out any None models (from unsupported types)
                self.models_B = [model for model in self.models_B if model is not None]
                
                # Special case for 'ls' model type to calculate sigmahat
                if self.model_type == 'ls' and len(self.models_B) > 0:
                    # Calculate sigmahat using the first model as a representative
                    model_b = self.models_B[0]
                    self.sigmahat = np.mean(np.square(model_b.predict(self.x_train)-self.y_train))**0.5
                    
            except Exception as e:
                print(f"Parallel fitting failed with error: {str(e)}")
                print("Falling back to sequential fitting...")
                
                # Fallback to sequential fitting if parallel fails
                self.models_B = []
                for b in range(self.B):
                    model_b = self._fit_single_bootstrap(x_train, y_train, b, self.subsample)
                    if model_b is not None:
                        self.models_B.append(model_b)
                        
                # Calculate sigmahat for 'ls' model type
                if self.model_type == 'ls' and len(self.models_B) > 0:
                    model_b = self.models_B[0]
                    self.sigmahat = np.mean(np.square(model_b.predict(self.x_train)-self.y_train))**0.5

        # Handle non-bootstrapping methods (CQR-base)
        if not(self.bootstrapping_for_uacqrp):
            if self.model_type=='rfqr':
                ...
                self.cqr_base_model = RandomForestQuantileRegressor(default_quantiles=[self.q_lower/100, self.q_upper/100], random_state=self.random_state, 
    ...
    def calibrate(self, x_calib, y_calib, inject_noise=False, cond_exp=False, noise_sd_fn=False):
        ...
        if self.bootstrapping_for_uacqrp:
            cqr_pred_calib = calib_sorted_q_lower_ests_df[int(self.B/2)]
            cqr_pred_calib_upper = calib_sorted_q_upper_ests_df[int(self.B/2)]
            cqr_scores_upper = self.transform(calib_sorted_q_upper_ests_df['truth']) - self.transform(cqr_pred_calib_upper)
            cqr_scores_lower = self.transform(cqr_pred_calib)  - self.transform(calib_sorted_q_upper_ests_df['truth'])
            calib_interval_widths = self.transform(cqr_pred_calib_upper) - self.transform(cqr_pred_calib)
        else:
            cqr_predictions_calib = self.cqr_base_model.predict(x_calib)
            cqr_scores_upper = self.transform(calib_sorted_q_upper_ests_df['truth']) - self.transform(cqr_predictions_calib[:, -1])
            cqr_scores_lower = self.transform(cqr_predictions_calib[:, 0])  - self.transform(calib_sorted_q_upper_ests_df['truth'])
            calib_interval_widths = self.transform(cqr_predictions_calib[:, -1]) - self.transform(cqr_predictions_calib[:, 0])
        ...
        if self.oracle_g:
            cqr_oracle_predictions_calib = self.cqr_base_model.predict(x_calib)
            cqr_model_lower_calib = cqr_oracle_predictions_calib[:, 0]
            cqr_model_upper_calib = cqr_oracle_predictions_calib[:, -1]
    ...
    def predict(self, x_test):
        ...
        if self.oracle_g:
            cqr_oracle_predictions_test = self.cqr_base_model.predict(x_test)
            cqr_model_lower = cqr_oracle_predictions_test[:, 0]
            cqr_model_upper = cqr_oracle_predictions_test[:, -1]
        ...
        if self.bootstrapping_for_uacqrp:
            ...
            cqr_predictions_test = self.cqr_base_model.predict(x_test)
            cqr_model_lower = cqr_predictions_test[:, 0]
            cqr_model_upper = cqr_predictions_test[:, -1]    
    ...
    def calculate_conditional_coverage(self, cond_exp=None, noise_sd_fn=None, plot=True, sorting_column=0, round_to=2, title=None,
        ...
        if cond_exp:
            ...
            if column_by_feature_importance:
                sorting_column = np.argmax(self.cqr_base_model.feature_importances_)
            if ecdf:
    ...
    def __predict_individual_model(self, x_data, model,lower=True):
        if lower:
            i = 0
        else:
            i = 1
        ...
        if self.model_type=='rfqr':
            ...
            return model.predict(x_data)[i]
        elif self.model_type=='linear':
            return model[i].predict(x_data)
        elif self.model_type=='lightgbm':
            return model[i].predict(x_data)
            ...
            return model.predict(x_data) + norm.ppf(self.q_lower/100) * self.sigmahat
        elif self.model_type == 'ls' and not(lower):
            return model.predict(x_data) + norm.ppf(self.q_upper/100) * self.sigmahat
        
        raise NotImplementedError(f"__predict_individual_model not fully implemented for model_type: {self.model_type}")

    def __predict_individual_model_batch(self, x_test, model, lower=True):
        """
        Make predictions for a whole batch of data with a single model.
        This function can be called in parallel for different models.
        
        Args:
            x_test: Test data features
            model: Fitted model
            lower: Whether to predict lower (True) or upper (False) quantile
            
        Returns:
            Array of predictions
        """
        return self.__predict_individual_model(x_test, model, lower=lower)
        
    def __predict_B_sorted(self, x_test, lower=True):
        """
        Generate predictions from all bootstrap models and sort them.
        Uses parallel processing for prediction when possible.
        ...
        Returns:
            Sorted array of predictions from all models
        """
        if self.model_type == 'neural_net' and not(self.bootstrapping_for_uacqrp):
            # Neural network with epoch-based ensemble doesn't need bootstrap loop
            q_preds = self.nn_model.predict(x_test, ensembling=self.B+1)[0 if lower else -1]
            # Append placeholder value for sorting
            q_preds = np.column_stack((q_preds, np.full(len(x_test), -np.inf if lower else np.inf)))
            q_preds.sort(axis=1)
            if lower:
                q_preds = q_preds[:,::-1]
            return q_preds
        
        # For other models, use parallel prediction if we have multiple models
        if len(self.models_B) > 20 and self.n_jobs != 1:  # Only parallelize if we have many models
            # Use fewer cores than for fitting to avoid memory issues
            n_parallel = max(1, self.n_jobs if self.n_jobs > 0 else min(joblib.cpu_count()//2, 4))
            ...
            try:
                ...
                # Convert to array and transpose to get (n_samples, n_models)
                q_preds = np.column_stack(all_preds)
            except Exception as e:
                print(f"Parallel prediction failed with error: {str(e)}. Falling back to sequential prediction.")
                # Fallback to sequential prediction
                q_preds = np.zeros((len(x_test), len(self.models_B)))
                for t, model in enumerate(self.models_B):
                    q_preds[:,t] = self.__predict_individual_model(x_test, model, lower=lower)
        else:
        ...
        # Append placeholder value for sorting and sort
        q_preds = np.column_stack((q_preds, np.full(len(x_test), -np.inf if lower else np.inf)))
        q_preds.sort(axis=1)
        if lower:
            q_preds = q_preds[:,::-1]
