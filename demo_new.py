...
- **ALEATORIC-R**: aleatoric QRF intervals centered on PCS median, conformally calibrated (the $\lambda=1$ special case of CLEAR, also called CQR-R+ in the paper)
- **Aleatoric**: plain CQR, QRF trained on raw targets, no epistemic component
- **PCS**: standard multiplicative PCS calibration (epistemic-only, no aleatoric model)
"""
...
# --- ALEATORIC-R baseline (CQR-R+): aleatoric intervals centred on PCS median ---
...
- **ALEATORIC-R**: aleatoric QRF on residuals, centred on PCS median (CQR-R+ in paper)
- **Aleatoric**: plain CQR — QRF trained on raw targets, no epistemic component
- **PCS**: multiplicative calibration, epistemic-only
"""
